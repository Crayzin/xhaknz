...
exports.run = async (client, message, args) => {
    ...
    if(!message.member.hasPermission('ADMINISTRATOR')){
        return message.channel.send('<:no:930170786457722920> Çekiliş bitirmek için **__Yönetici__* yetkisine sahip olmanız gerekmektedir.');
    }

    // If no message ID or giveaway name is specified
    if(!args[0]){
        return message.channel.send('<:no:930170786457722920> Bir mesaj IDsi girin');
    }
